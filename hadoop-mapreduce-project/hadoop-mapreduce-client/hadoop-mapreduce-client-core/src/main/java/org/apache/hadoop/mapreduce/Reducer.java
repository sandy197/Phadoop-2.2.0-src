...
//srkandul
	...
	if(isFirst){/*
		tp.set_affinity(context.getCurrentKey().index1 +
					context.getCurrentKey().index2 +
						context.getCurrentKey().index3);
	*/}
...
//srkandul
	System.out.println("Exiting reduce run tid:"+tp.get_thread_id() + ", pinned to core: " + tp.get_affinity());
	System.out.println("ReduceTime(ns): "+ (end - start));
    } finally {
      cleanup(context);
    }
  }
