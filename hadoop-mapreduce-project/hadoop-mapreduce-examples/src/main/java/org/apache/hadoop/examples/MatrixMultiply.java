...
public class MatrixMultiply {
	...
	private static long reduceTime = 0;
			...
			switch (strategy) {
				case 1:
					if (matrixA) {
						key.index1 = i/IB;
						key.index2 = k/KB;
						//key.coreId = key.index1 + key.index2;
						key.m = 0;
						value.index1 = i % IB;
						value.index2 = k % KB;
...
/*
		private int buildSparse(int[] NZ, int[]CI, int[]RS, int rowDim, int colDim,
			Iterable<Value> valueList){
	//TODO:use array lists for NZ and CI
		//populate NZ, CI, RS
			int prevRowCoord = -1;
			int i = 0;
			// initialize RS to -1s
			for(int j = 0; j < RS.length; j++)
				RS[j]= -1;
			for(Value value : valueList){
			// NZ can be populated from valueList
			NZ[i] = value.v;
			// CI from valueList
			CI[i] = value.index2;
			// RS from the same with special handling for the all zeros case
			if(prevRowCoord != value.index1){
			// record the index whenever there is a change of the row-coordinate
				RS[value.index1] = i;
				prevRowCoord = value.index1;
			}
			i++;
			}
...
*/		
		private long multiplyAndEmit (Context context, int ib, int jb)
			throws IOException, InterruptedException 
		{
			//System.out.println("************Mutiplying values*******");
			int ibase = ib*IB;
			int jbase = jb*JB;
			long multiplyTime = 0;
			long writingTime = 0;
			...
			throws IOException, InterruptedException 
		{
			...
			System.out.println("Strategy:" + strategy + "Reduce task being pinned to: "+ coreID);
			//System.out.println("************reduce task start*******");
			...
			switch (strategy) {
				case 1:
					...
					if (key.m == 0) {
						...
						build(A, aRowDim, aColDim, valueList, context);
					} else {
						if (ib != sib || kb != skb) return;
						bColDim = getDim(jb, lastJBlockNum, JB, lastJBlockSize);
						build(B, aColDim, bColDim, valueList, context);
				...
				case 2:
					...
					if (jb < 0) {
						...
						aRowDim = getDim(ib, lastIBlockNum, IB, lastIBlockSize);
						aColDim = getDim(kb, lastKBlockNum, KB, lastKBlockSize);
						nz1 = build(A, aRowDim, aColDim, valueList, context);
						density1 = (float)nz1/(aRowDim * aColDim);
					} else {
						if (ib != sib || kb != skb) return;
						bColDim = getDim(jb, lastJBlockNum, JB, lastJBlockSize);
						nz2 = build(B, aColDim, bColDim, valueList, context);
						density2 = (float)nz2/(aColDim * bColDim);
						System.out.println("Density of A: "+ density1 + " Density of B: "+density2);
						long start = System.nanoTime();
						writeTime += multiplyAndEmit(context, ib, jb);
						long end = System.nanoTime();
						System.out.println("MultiplyTime(ns): "+ (end - start));
						reduceTime += (end - start);
						System.out.println("TotalReduceTime(ns): "+ reduceTime);
						System.out.println("ComputeTime(ns): "+ writeTime);
					}
					break;
				case 3:
					kb = key.index1;
		...
		throws Exception
	{
		...
		System.out.println("Number of reduce tasks for job1 set to: "+ conf.getInt("MatrixMultiply.R1", 0));
